...
{strip}
    ...
    {/if}
    <input type="hidden" name="action" value="PopupSave" />
    ...
    <div class="form-div contents-bottomscroll">
        {foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$RECORD_STRUCTURE name="EditViewBlockLevelLoop"}
            ...
            <table class="table table-bordered blockContainer showInlineTable equalSplit ">
                <thead>
                <tr>
                    <th class="blockHeader" colspan="1">{vtranslate($BLOCK_LABEL, $MODULE)}</th>
                </tr>
                </thead>
                <tbody>
                <tr>
                ...
                <tr>
                    ...
                    <td class="fieldLabel {$WIDTHTYPE}" >
                        {if $isReferenceField neq "reference"}<label class="muted pull-left marginRight10px">{/if}
                            ...
                            {if $isReferenceField eq "reference"}
                                ...
                                {else}
                                    <label class="muted pull-left marginRight10px">{if $FIELD_MODEL->isMandatory() eq true} <span class="redColor">*</span> {/if}{vtranslate($FIELD_MODEL->get('label'), $MODULE)}</label>
                                {/if}
                    ...
                    {if $FIELD_MODEL->get('uitype') neq "83"}
                        <td class="fieldValue {$WIDTHTYPE}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20'} colspan="1" {assign var=COUNTER value=$COUNTER+1} {/if}>
                            <div class="row-fluid">
                        <span class="span10">
        ...
        <table>
            {if $MODULE eq 'Events' }
                <thead>
                <tr>
                    <th class="blockHeader" colspan="2">{vtranslate('LBL_INVITE_USER_BLOCK', $MODULE)}</th>
                </tr>
                </thead>
                <tbody>
                <tr>
