...
class QueryGenerator {
	...
	public function getFromClause() {
		...
		foreach ($this->fields as $fieldName) {
			...
			} elseif($field->getFieldDataType() == 'owner' && $baseModule == 'Dailyreports') {
        ...
        foreach ($this->conditionals as $index=>$conditionInfo) {
			...
			foreach ($valueSqlList as $valueSql) {
				if (in_array($fieldName, $this->referenceFieldList)) {
                    ...
                    } elseif($this->module == 'Dailyreports' && $fieldName == 'assigned_user_id') {
						$concatSql = getSqlForNameInDisplayFormat(array('first_name'=>"vtiger_usersassigned_user_id.first_name",'last_name'=>"vtiger_usersassigned_user_id.last_name"), 'Users');
						$fieldSql .= "$fieldGlue (trim($concatSql) $valueSql or "."vtiger_groupsassigned_user_id.groupname $valueSql)";
                    }else{
