								...
								{if !empty($src)}
									...
									{else}
										...
										{if strlen($first_name) > 0 && strlen($last_name) > 0 }
											{assign var=title value="`$last_name``$first_name`"}
										{else}
											{assign var=title value=$first_name}
										{/if}
										{if empty($title)}
											{assign var=title value=$USER_MODEL->get('last_name')}
										{/if}
									<span class="dropdown-toggle" data-toggle="dropdown" href="#">
                                        <a id="menubar_item_right_{$title|stripslashes}"  class="userName textOverflowEllipsis" title="{$title|stripslashes}"><strong>{$title|stripslashes}</strong>&nbsp;<i class="caret"></i> </a> </span>
									{/if}
									{if !empty($childLinks)}
