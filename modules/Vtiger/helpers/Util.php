...
class Vtiger_Util_Helper {
	...
	public static function formatDateIntoStrings($date, $time = false) {
		...
		if ($dateInUserFormat == $today) {
            ...
            $formatedDateYear  = "";
            $formatedDateMonth = date('n月', $date);
            $formatedDateDay   = date('d日', $date);
            $formatedDateDOW   = '(' . vtranslate('LBL_'.date('D', $date)) . ')';
            $formatedDateTime  = $displayTime;

            if (date('Y', $date) != date('Y')) {
				$formatedDateYear  = date('Y年 ', $date);
			}

            $formatedDate = "{$formatedDateYear}{$formatedDateMonth} {$formatedDateDay} {$formatedDateDOW}";
	...
	public static function formatDateTimeIntoDayString($dateTime) {
		...
		//Adding date details, time details
        $formatedDateYear  = date('Y年', $date);
        $formatedDateMonth = date('n月', $date);
        $formatedDateDay   = date('d日', $date);
        $formatedDateDOW   = '(' . vtranslate('LBL_'.date('D', $date)) . ')';
		...
		$formatedDate = "{$formatedDateYear} {$formatedDateMonth} {$formatedDateDay} {$formatedDateDOW} {$formatedDateTime}";
	...
	public static function sanitizeUploadFileName($fileName, $badFileExtensions) {
		...
		$fileName = preg_replace('/[\/<>\?\*:"<>\| ]/', '', $fileName);//日本語が文字化けするのでこっちで。
