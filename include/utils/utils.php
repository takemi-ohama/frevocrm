 ...
 * @return array
 */
function getContactList($activityid) {
	global $adb;
	$sql =<<<EOF
select
	con.contactid,
	con_crm.label
from
	vtiger_cntactivityrel con_act_rel,
	vtiger_contactdetails con,
	vtiger_crmentity con_crm
where
	con_act_rel.activityid = $activityid and
	con_act_rel.contactid = con.contactid and
	con.contactid = con_crm.crmid and
	con_crm.deleted = 0
EOF;
	...
	for($i=0; $i<$noOfRows; ++$i) {
		...
		$result_records[] = array("id" => $row["contactid"], "name" => $row["label"]);
	}
