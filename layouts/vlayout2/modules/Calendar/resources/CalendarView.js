...
jQuery.Class("Calendar_CalendarView_Js",{
	...
	dayClick : function(date, allDay, jsEvent, view){
		...
		this.getCalendarCreateView().then(function(data){
			...
			var url = 'index.php?module=Calendar&view=PopupEdit&mode=Events&date_start='+startDateString+'&time_start='+startTimeString+'&due_date='+endDateString+'&time_end='+endTimeString;
	...
	registerCalendar : function(customConfig) {
		...
		var config = {
           ...
           eventDrop : function( event, dayDelta, minuteDelta, allDay, revertFunc, jsEvent, ui, view ) {
			...
			eventMouseover : function(calEvent, jsEvent, view) {
				...
				if( !document.getElementById('tooltip_'+calEvent.id) ) {
					$('body').prepend(calEvent.tooltip);
					xOffset = 30 + $('#tooltip_'+calEvent.id).height();
					yOffset = -20;
					$('#tooltip_'+calEvent.id)
		...
		jQuery('<span class="pull-left">'+
			'<a class="add-event btn addButton" href="index.php?module=Calendar&view=PopupEdit&mode=Events">'+ app.vtranslate('LBL_ADD_EVENT',"Vtiger") +'</button>'+
			'<a class="add-event btn addButton" href="index.php?module=Calendar&view=PopupEdit&mode=Calendar">'+ app.vtranslate('LBL_ADD_TASK') +'</button>'+
			'</span>')
