			...
			{foreach item=RECORD key=KEY from=$ACTIVITIES}
				...
				<div class="activityEntries">
					...
					<div class="summaryViewEntries">
						...
						{if $DETAILVIEW_PERMITTED == 'yes'}<a id="full_activity_{$RECORD->getId()}" data-tooltip="#html_full_activity_{$RECORD->getId()}" href="{$RECORD->getDetailViewUrl()}" >{$RECORD->get('subject')}</a>{else}<span id="full_activity_{$RECORD->getId()}" data-tooltip="#html_full_activity_{$RECORD->getId()}">{$RECORD->get('subject')}</span>{/if}&nbsp;
						...
						{include file='ActivityBalloon.tpl'|@vtemplate_path activityid=$RECORD->getId() activitytype=$RECORD->get("activitytype") gravity="east"}
					</div>
				</div>
                                <hr>
			{/foreach}
