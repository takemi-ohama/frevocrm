...
class Calendar_Feed_Action extends Vtiger_BasicAjax_Action {
	...
	protected function pullEvents($start, $end, &$result, $userid = false,$color = null,$textColor = 'white') {
		...
		while($record = $db->fetchByAssoc($queryResult)){
			...
			if(!$currentUser->isAdminUser() && $visibility == 'Private' && $userid && $userid != $currentUser->getId()) {
				...
				$item['url']   = sprintf('index.php?module=Calendar&view=Detail&mode=Events&tab=no&record=%s', $crmid);
			}
			$balloon_html = "";
			$balloon_html .= "<span class='title'>".vtranslate('LBL_SUBJECT',"Events")."</span>：".$record['subject']."<br>";
			$balloon_html .= "<span class='title'>".vtranslate('Assigned To',"Events")."</span>：".getUserFullName($record['smownerid'])."<br>";
			$balloon_html .= "<span class='title'>".vtranslate('Start Date & Time',"Events")."</span>：".Vtiger_Util_Helper::formatDateTimeIntoDayString($record['date_start']." ".$record['time_start'])."<br>";
			...
			$item['tooltip'] = '<div class="balloon" id="tooltip_'.$crmid.'">'.$balloon_html.'</div>';
	...
	protected function pullTasks($start, $end, &$result, $color = null,$textColor = 'white') {
		...
		while($record = $db->fetchByAssoc($queryResult)){
			...
			$item['url']   = sprintf('index.php?module=Calendar&view=Detail&mode=Calendar&tab=no&record=%s', $crmid);
			$item['color'] = $color;
			$item['textColor'] = $textColor;
            $item['module'] = $moduleModel->getName();
			$balloon_html = "";
			$balloon_html .= "<span class='title'>".vtranslate('LBL_SUBJECT',"Calendar")."</span>：".$record['subject']."<br>";
			$balloon_html .= "<span class='title'>".vtranslate('Assigned To',"Calendar")."</span>：".getUserFullName($record['smownerid'])."<br>";
			$balloon_html .= "<span class='title'>".vtranslate('Start Date & Time',"Calendar")."</span>：".Vtiger_Util_Helper::formatDateTimeIntoDayString($record['date_start']." ".$record['time_start'])."<br>";
			$balloon_html .= "<span class='title'>".vtranslate('End Date',"Calendar")."</span>：".Vtiger_Util_Helper::formatDateIntoStrings($record['due_date'])."<br>";
			$balloon_html .= "<span class='title'>".vtranslate('LBL_ACTIVITY_TYPES',"Calendar")."</span>：".vtranslate($record['activitytype'],"Events")."<br>";
			$balloon_html .= "<span class='title'>".vtranslate('Status',"Vtiger")."</span>：".vtranslate($record['taskstatus'],"Calendar")."<br>";
			$balloon_html .= "<span class='title'>".vtranslate('Description',"Events")."</span>：<br>".vtranslate($record['description'],"Calendar");
			$item['tooltip'] = '<div class="balloon" id="tooltip_'.$crmid.'">'.$balloon_html.'</div>';
			$result[] = $item;
		}
	}
