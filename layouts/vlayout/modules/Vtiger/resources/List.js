...
jQuery.Class("Vtiger_List_Js",{
	...
	getRecordsCount : function(){
		...
		if(recordCountVal != ''){
			...
			var search_params = "";
			if (location.href.indexOf("search_params") > -1) {
				var params = location.href.split("?")[1].split("&");
				for (var i = 0; i < params.length; i++) {
					if (params[i].indexOf("search_params") > -1) {
						var p = params[i].replace("search_params=", "").replace("+", "%20");
						search_params = JSON.parse(decodeURIComponent(p));
					}
				}
			}

			postData.search_params = JSON.stringify(search_params);
			//postData.search_params = JSON.stringify(this.getListSearchParams()); //deleted for patch
	...
	getPageJumpParams : function(){
		...
		var search_params = "";
		if (location.href.indexOf("search_params") > -1) {
			var temp = location.href.split("?")[1].split("&");
			for (var i = 0; i < temp.length; i++) {
				if (temp[i].indexOf("search_params") > -1) {
					var p = temp[i].replace("search_params=", "").replace("+", "%20");
					search_params = decodeURIComponent(p);
				}
			}
		}
