            ...
            <thead>
                <tr class="listViewHeaders">
                    {foreach item=HEADER_FIELD from=$RELATED_HEADERS}
                            ...
                            {elseif $HEADER_FIELD->get('column') eq 'time_end'}
                            {else}
                    ...
                    {foreach item=HEADER_FIELD from=$RELATED_HEADERS}
                        {assign var=RELATED_HEADERNAME value=$HEADER_FIELD->get('name')}
                            ...
                            {if $HEADER_FIELD->isNameField() eq true or $HEADER_FIELD->get('uitype') eq '4'}
								{if $HEADER_FIELD->getModuleName() == "Calendar" and $HEADER_FIELD->getName() == "subject"}
                                <a id="full_activity_{$RELATED_RECORD->getId()}" data-tooltip="#html_full_activity_{$RELATED_RECORD->getId()}" href="{$RELATED_RECORD->getDetailViewUrl()}">{$RELATED_RECORD->getDisplayValue($RELATED_HEADERNAME)}</a>
								{include file='ActivityBalloon.tpl'|@vtemplate_path activityid=$RELATED_RECORD->getId() activitytype=$RELATED_RECORD->get("activitytype") gravity="west"}
								{else}
                            ...
                            {else}
                                {if $HEADER_FIELD->getModuleName() == "Calendar" and $HEADER_FIELD->getName() == "subject"}
                                <span id="full_activity_{$RELATED_RECORD->getId()}" data-tooltip="#html_full_activity_{$RELATED_RECORD->getId()}">{$RELATED_RECORD->getDisplayValue($RELATED_HEADERNAME)}</span>
								{include file='ActivityBalloon.tpl'|@vtemplate_path activityid=$RELATED_RECORD->getId() activitytype=$RELATED_RECORD->get("activitytype") gravity="west"}
                                {else}
